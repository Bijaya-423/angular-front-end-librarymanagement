...
export class BorrowListComponent implements OnInit {
  ...
  loadBorrows() {
    ...
    this.borrowService.getAllBorrows().subscribe({
      ...
      error: (err) => {
        ...
        this.showError('Failed to load borrow records');
        this.loading = false;
      }
    });
  }
  ...
  approve(id: number) {
    this.borrowService.approveBorrow(id).subscribe({
      next: () => {
        this.showSuccess('✅ Borrow approved!');
        this.loadBorrows();
      },
      error: (err) => this.showError(
        err.error?.error || 'Approve failed')
  ...
  reject(id: number) {
    this.borrowService.rejectBorrow(id).subscribe({
      next: () => {
        this.showSuccess('✅ Borrow rejected!');
        this.loadBorrows();
      },
      error: (err) => this.showError(
        err.error?.error || 'Reject failed')
    ...
    this.borrowService.getBorrowById(id).subscribe({
      next: (b) => {
        alert(
          ...
          `Due Date: ${b.dueDate}\n` +
          `Return Date: ${b.returnDate || 'Not returned yet'}\n` +
          `Status: ${b.status}`
        );
      },
      error: () => this.showError('Failed to load borrow details')
    });
  }
